...
namespace GetHash
{
  class Program
  {
    static void Main(string[] args)
    {
      ...
      string exeSHA1Hash = "";
      string zipSHA1Hash = "";
      string exeSHA256Hash = "";
      string zipSHA256Hash = "";
      ...
      foreach (string file in FileList)
      {
        ...
        //-----------------------------------
        // SHA-256
        string sha256 = getSha256Hash(file);
        HashFilePath = Path.GetDirectoryName(file) + "\\" + Path.GetFileName(file) + ".sha256";
        File.WriteAllText(HashFilePath, sha1, Encoding.UTF8);
        if (ext == ".exe")
        {
          exeSHA256Hash = sha256;
        }
        else
        {
          zipSHA256Hash = sha256;
        }
      ...
      using (StreamWriter sw = new StreamWriter(Path.Combine(dirpath, basefilename + ".json"), false, Encoding.UTF8))
      {
        ...
        sw.WriteLine("        \"sha256\" : \"" + exeSHA256Hash + "\"");
        sw.WriteLine("      },");
        sw.WriteLine("      {");
        ...
        sw.WriteLine("        \"sha256\" : \"" + zipSHA256Hash + "\"");
        sw.WriteLine("      }");
        sw.WriteLine("    ]");
        sw.WriteLine("");
        sw.WriteLine("    \"archives\" : [");
